// Mostly the original Shower.js, wrapped in a function to defer execution
function startSlideshow() {
	...
	var hash = url.hash;
	url.hash = null;
	setTimeout(function() {url.hash = hash;}, 0);
	...
	function increaseInnerNavigation(slideNumber) {
		...
		var $inner = $(getSlideHash(slideNumber) + ' .inner:not(.active)');
		if ($inner.length > 0) {
